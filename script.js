function initialTemperature() {
  ...
  axios.get(apiUrl).then((response) => {
    ...
    // Ensure humidity and wind speed are properly updated
...
let newYorkWeather = axios.get(`https://api.openweathermap.org/data/2.5/weather?q=new%20york&units=metric&appid=22fe714020fda5e1b1931cc750b203ae`).then(displayWeatherInfoNewYork);
...
function displayWeatherInfo(response) {
  ...
  // Updated humidity and wind speed
  const humidity = response.data.main.humidity;
  const windSpeed = Math.round(response.data.wind.speed);
